...
export function ProposalRenderer({
  ...
  const contentSections = sections
    .filter((s) => s.type !== 'cover' && s.type !== 'table_of_contents' && s.type !== 'signature' && s.content)
  ...
  const resolvedClientName = cover?.clientName || companyName || ''
  ...
  return (
    <div className="mx-auto max-w-4xl py-8 space-y-0 print:bg-white">
      {/* ===== PAGE 1: COVER PAGE ===== */}
      ...
      {/* NOTE: Signature section is handled by the SignatureSection component in the client view */}
    </div>
  )
}
